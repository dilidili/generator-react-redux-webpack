import React, {PropTypes} from 'react'
import {connect} from 'react-redux'
import {bindActionCreators} from 'redux'
import ReactCanvas from 'react-canvas'
import WB from './WB'
import styles from './WBList.scss'
import classNames from 'classnames'

const REPOST_SRC = "data:image/png;base64,"
const LIKE_SRC = "data:image/png;base64,"
const COMMENT_SRC = "data:image/png;base64,"
const BUTTON_SRC = "data:image/png;base64,"
const HEADER_LAYER_INDEX = 3
const HEADER_LAYER_BUTTON_INDEX = 4

const WBListComponent = React.createClass({
	// getter
	getListViewStyle: function() {
		return {
			top: 0,
			left: 0,
			width: this.props.size.width,
			height: this.props.size.height,
		}
	},
	getNumberOfPages: function(){
		return this.props.wb.length
	},
	getPageHeight: function(){
		return this.props.size.height
	},
	getGradientStyle: function(){
		return{
			top: 0,
			left: 0,
			width: this.props.size.width,
			height: 50,
			zIndex: HEADER_LAYER_INDEX,
		}
	},
	getGradientColors: function(){
		return [{
			color: "transparent",
			position: 1
		}, {
			color: "rgba(0, 0, 0, 0.2)",
			position: 0
		}]
	},
	getHeaderButtonStyle: function(){
		const gs = this.getGradientStyle()
		return {
			top: gs.height*0.15,
			left: gs.width - gs.height*0.85,
			width: gs.height*0.7,
			height: gs.height*0.7,
			zIndex: HEADER_LAYER_BUTTON_INDEX,
		}
	},

	// lifecycle
	propTypes: {
		size: PropTypes.object.isRequired,
		wb: PropTypes.array.isRequired,
	},
	renderWb: function(wbIndex, scrollTop){
		const wb = this.props.wb[wbIndex] 
		const pageScrollTop = wbIndex * this.getPageHeight() - scrollTop
	    return (
			<WB
				width={this.props.size.width}
				height={this.props.size.height}
				wb={wb}
				scrollTop={pageScrollTop}/>
	    )
	},
	renderWbControlPanel: function(){
		return (
			<div className={styles.wbControlPanel}>
				<img className={classNames(styles.icon, styles.repost)} src={REPOST_SRC}/>	
				<img className={classNames(styles.icon, styles.like)} src={LIKE_SRC}/>	
				<img className={classNames(styles.icon, styles.comment)} src={COMMENT_SRC}/>	
			</div>
		)	
	},
	renderHeader: function(){
		return (
			<ReactCanvas.Group style={this.getGradientStyle()}>
				{/*<ReactCanvas.Gradient 
					style={this.getGradientStyle()} 
					colorStops={this.getGradientColors()}
				/>*/}
				<ReactCanvas.Image style={this.getHeaderButtonStyle()} src={BUTTON_SRC} fadeIn={true} useBackingStore={true}/>
			</ReactCanvas.Group>
		)
	},
	render: function(){
		// avoid useless render
		if (this.props.wb.length<=0) return null

		const {size} = this.props

		return (
			<div>
				<ReactCanvas.Surface top={0} left={0} width={size.width} height={size.height}>
					{this.renderHeader()}
					<ReactCanvas.ListView
						style={this.getListViewStyle()}
						snapping={true}
						scrollingDeceleration={0.93}
						scrollingPenetrationAcceleration={0.13}
						numberOfItemsGetter={this.getNumberOfPages}
						itemHeightGetter={this.getPageHeight}
						itemGetter={this.renderWb}
					/>
				</ReactCanvas.Surface>
				{this.renderWbControlPanel()}
			</div>
		)
	},
})

export default WBListComponent